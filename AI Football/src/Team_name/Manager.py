...
def team_properties():
    ...
    properties['team_name'] = "Весели Лини"
    ...
    properties['radius_points'] = (5, 0.5, 1)
    properties['max_acceleration_points'] = (15, 40, 14)
    properties['max_speed_points'] = (30, 34.5, 14.5)
    properties['shot_power_points'] = (15, 25.5, 6.5)
...
def player1(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    elif ((ball_pos[0] <= 341.5 and your_side == "left") or (ball_pos[0] >= 1024.5 and your_side == "right")) and 343 < ball_pos[1] < 578:
...
def player2(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    if ((ball_pos[0] >= 1024.5 and your_side == "left") or (
            ball_pos[0] <= 341.5 and your_side == "right")) and ball_acquired:
        attacker['shot_request'] = True
        attacker['shot_power'] = player["shot_power_max"]
...
def player3(our_team, their_team, ball, your_side, half, time_left, our_score, their_score):
    ...
    if (ball_position[0] > 683 and your_side == "left") or (ball_position[0] <= 683 and your_side == "right"):
        ...
        slimmest_opponent = find_slimmest_opponent(their_team, your_side)
        if 343 < slimmest_opponent["y"] < 578:
            helper["alpha"] = calculate_angle(player_position, (slimmest_opponent["x"], slimmest_opponent["y"]))
        else:
            helper["alpha"] = calculate_angle(player_position, goalkeeper_position)
        return helper
    else:
